...
# # Uncomment to save your model (Uncomment saver.save line as well)
# saver = tf.train.Saver()

cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits
    (labels=y_, logits=y_conv))
# Using ADAM optimizer (Kingma and Ba, 2014) to compute gradients
...
# Trains the network with 20000 training iterations, each having 50 training 
#   examples. We log the training accuracy at every 100th iteration.
for i in range(20000):
    batch = mnist.train.next_batch(50)
    if i%100 == 0:
        train_accuracy = accuracy.eval(feed_dict={
            x:batch[0], y_: batch[1], keep_prob: 1.0})
        print("Step %d, training accuracy %.2f%%" % (i, train_accuracy*100))
    train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5})

# # Uncomment to save your model
# saver.save(sess, './savefiles/my-model')
